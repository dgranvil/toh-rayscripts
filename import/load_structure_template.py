...
class load_structure_template:
    ...
    def load_template(self, selected_careplan, selected_site): 
        # Decide which template to use  
        try:
            template = selected_careplan + ' SS'
            ...
            show_result_message('Unable to find matching structure template. Please load manually.')
        
        if selected_site == 'Head and Neck':
...
def do_task(**options):
    load_structure_template().load_template(selected_careplan = options['selected_careplan'], selected_site = options['selected_site'])
